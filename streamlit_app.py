...
if file is not None:
    ...
    def data_maker(file):
        ...
        return door_dict_pass,door_dict_drive, time_df_dict, df_door_fleet, df_door_train, df_door_fleet_driv,df_door_train_driv , df_door_time_fleet, df_door_time_train, df_station

    door_dict_pass,door_dict_drive , time_df_dict, df_door_fleet, df_door_train, df_door_fleet_driv,df_door_train_driv ,  df_door_time_fleet, df_door_time_train, df_station = data_maker(file)
    ...
    with tab1:
        ...
        ca1col1.metric("Passengers Doors | Mean", df_door_fleet["total"][0])
        ca1col2.metric("Drivers Doors | Mean", df_door_fleet_driv["total"][0])
        ...
        ca2col1.metric("Passengers Doors | Mean", df_door_fleet["total"][1])
        ca2col2.metric("Drivers Doors | Mean", df_door_fleet_driv["total"][1])
        ...
        cola.metric("Average time between passenger doors openings", f'{df_door_time_fleet["Open"]["total"][0]} s')
        colb.metric("Average time between passenger doors openings", f'{df_door_time_fleet["Open"]["total"][1]} s')
        with cola.expander("See for results per trains"):
        ...
        cola.metric("Average time between passenger doors closings", f'{df_door_time_fleet["Close"]["total"][0]} s')
        colb.metric("Average time between passenger doors closings", f'{df_door_time_fleet["Close"]["total"][1]} s')
        with cola.expander("See for results per trains"):
    ...
    with tab2:
        ...
        if option_type == "Time between operation":
            option_op = col3.selectbox("Operation", ("Open", "Close", "Any"))
            st.dataframe(time_df_dict[option_op][option_day].round(decimals=1), hide_index=True)

        if option_type == "Door operation count":
            option_door = col3.selectbox("Door type", ("Driver Door", "Passenger Door"))
            if option_door == "Passenger Door":
                st.dataframe(door_dict_pass[option_day], hide_index=True)
            elif option_door == "Driver Door":
